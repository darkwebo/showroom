<?PHP
include "D:\wamp64\www\auto\TheAutocars\config.php";
...
class cars{
	...
	function affichercar2($name){
		$sql="SELECT * FROM car where ID=:name";
		$db = config::getConnexion();
        $req=$db->prepare($sql);
		$req->bindValue(':name',$name);
		try{
	...
	function modifiercar($car,$id){
		$sql="UPDATE car SET make=:make,model=:model,price=:price,body_type=:body_type,description=:description,year=:year,mileage=:mileage,cv=:cv,fuel_type=:fuel_type,transmission=:transmission,photo=:photo    WHERE id=:id";
