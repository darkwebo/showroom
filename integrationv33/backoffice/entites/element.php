...
class User{
	...
	public function Logedin($login,$pwd)
	{
		...
		$req="select * from client where id='$login' && password='$pwd'";
		$rep=$db->query($req);
		return $rep->fetchAll();
	}
}
...
class Confirmer{
	public function ajouterReservation($Commande)
	{
		$db = Config::getConnexion();
		$sql = $db->prepare('INSERT INTO Commande (id, article, `date`, date_expire, prixUnitaire, quantite, total, id_C) VALUES (:id, :article, now(), now()+INTERVAL 2 DAY, :prixUnitaire, :Quantite, :prixTotal, :id_C);');
	...
	{
		$db = Config::getConnexion();
		$req = "DELETE FROM commande WHERE date_expire < now()";
    ...
    public function modifierCommande($Commande)
	{
        ...
        */
		...
		$sql->bindValue(':prixTotal', $Commande->getTotal());
		$sql->execute();
		header("Location: ../afficher_detail.php");
		exit;
	}
